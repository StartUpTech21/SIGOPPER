...
$my = mysqli_query($koneksi, "SELECT * FROM tempat JOIN kategori USING(kode_kategori) JOIN galeri USING(kode_tempat) WHERE kategori.nama_kategori='Modern' AND (tempat.nama_tempat LIKE '%$search%' OR tempat.alamat_tempat  LIKE '%$search%' OR kategori.nama_kategori LIKE '%$search%') GROUP BY tempat.kode_tempat ASC LIMIT $per_laman OFFSET $awal");
?>
